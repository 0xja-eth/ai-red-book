...
def generate_output_images(files):
    for i in range(pic_count):
        if pic_count == 1:
            output_file = os.path.join(OUTPUT_ROOT, "%d-pic.jpg" % pb.max_count)
        else:
            output_file = os.path.join(OUTPUT_ROOT, "%d-pic-%d.jpg" % (pb.max_count, i + 1))

        if pic_mode == "9_pic":
            generate_9_pic(files, output_file)
        elif pic_mode == "4_pic":
            generate_4_pic(files, output_file)
        elif pic_mode == "single":
            generate_single(files, output_file)
